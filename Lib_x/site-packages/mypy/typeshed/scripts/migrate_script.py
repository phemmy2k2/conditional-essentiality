...
def add_stdlib_packages_from(
    subdir: str, packages: List[StdLibPackage], py_version: Optional[str]
) -> None:
...
def collect_third_party_packages() -> Tuple[
    List[ThirdPartyPackage], List[ThirdPartyPackage]
]:
    ...
    add_third_party_packages_from(
        "third_party/3", third_party, py2_compatible=False, py3_compatible=True
    )
    add_third_party_packages_from(
        "third_party/2and3", third_party, py2_compatible=True, py3_compatible=True
    )
    ...
    if not package.is_dir:
        ...
        for dir_path, _, file_names in os.walk(package.path):
            for file_name in file_names:
                all_top_imports |= get_top_imported_names(
...
def copy_stdlib(
    packages: List[StdLibPackage], py2_packages: List[StdLibPackage]
) -> None:
    ...
    if py2_packages:
        ...
        for package in py2_packages:
            if not package.is_dir:
                ...
                shutil.copytree(
                    package.path, os.path.join(py2_stdlib_dir, package.name)
                )
...
def generate_metadata(package: ThirdPartyPackage, py2_packages: List[str]) -> str:
    ...
    if package.requires:
        distributions = [
            f'"types-{package_to_distribution.get(dep, dep)}"'
...
def copy_third_party(
    packages: List[ThirdPartyPackage], py2_packages: List[ThirdPartyPackage]
) -> None:
    ...
    for package in packages:
        ...
        if not os.path.isfile(metadata_file):
            ...
            with open(metadata_file, "w") as f:
                f.write(
                    generate_metadata(
                        package, [package.name for package in py2_packages]
                    )
                )
                f.write("\n")
        if not package.is_dir:
...
def main() -> None:
    ...
    py2_stdlib_names += [
        package.name for package in stdlib if package.py_version == "2.7"
    ]
    ...
    for package in third_party + py2_third_party:
        populate_requirements(
            package, stdlib_names, py2_stdlib_names, known_distributions
        )
