...
def main() -> None:
    ...
    files_to_test = determine_files_to_test(
        typeshed_location=typeshed_location, paths=args.files or subdir_paths
    )
    run_all_tests(
...
def run_pytype(
    *, filename: str, python_version: str, typeshed_location: str
) -> Optional[str]:
...
def determine_files_to_test(
    *, typeshed_location: str, paths: Sequence[str]
) -> List[Tuple[str, int]]:
    ...
    for path in paths:
        if os.path.isdir(path):
            for root, _, fns in os.walk(path):
                filenames.extend(
                    os.path.join(root, fn) for fn in fns if fn.endswith(".pyi")
                )
        else:
    ...
    for i, (f, version) in enumerate(files_to_test):
        stderr = (
            run_pytype(
                ...
                python_version="2.7"
                if version == 2
                else "{0.major}.{0.minor}".format(sys.version_info),
                typeshed_location=typeshed_location,
            )
            if not dry_run
            else None
