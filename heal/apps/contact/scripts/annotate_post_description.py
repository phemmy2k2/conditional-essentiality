...
def execmodel(unlab_dat, lab_dat, cuttoff, outdir, endloop, res_store, acc):
    ...
    # print(lab_dat['label'].value_counts())
    ...
    counter = Counter(y)
    print('Class distribution is %s' % str(counter))
    # train model
...
def perform_CML(userId, lab_path, unlab_path, outdir, threshold):
    ...
    while (unlab_dat.shape[0] > 1):
        print('\n' + 'Loop %d\n' % i)
        out, res_store, acc = execmodel(unlab_dat, lab_dat, threshold, outdir, endloop, res_store, acc)
        res_store = res_store
        acc = acc
        lab_dat = pd.concat([lab_dat, out], axis=0, sort=False)
...
def main(userId, lab_file, unlab_file, threshold=0.9):
    ...
    if type(threshold) == str:
        ...
        lab_path = ''.join((inp, lab_file))
        unlab_path = ''.join((inp, unlab_file))
